...
func (d *Deliverer) Deliver(ctx context.Context, ev outbox.PendingEvent) error {
	...
	case inventoryapp.EventCOGSPosted:
		...
		_, err = d.Client.PostCOGSJournal(ctx, &ledgerv1.PostCOGSJournalRequest{
			RestaurantId: req.RestaurantId, CreatedBy: req.CreatedBy, TicketId: req.SourceId,
	...
	case inventoryapp.EventReceiptPosted:
		...
		_, err = d.Client.PostReceiptJournal(ctx, &ledgerv1.PostReceiptJournalRequest{
			RestaurantId: req.RestaurantId, CreatedBy: req.CreatedBy, ReceiptId: req.SourceId,
			AccountingDate: req.AccountingDate, Lines: req.Lines,
		})
		return err
	case inventoryapp.EventWriteOffPosted:
		...
		_, err = d.Client.PostWriteOffJournal(ctx, &ledgerv1.PostWriteOffJournalRequest{
			RestaurantId: req.RestaurantId, CreatedBy: req.CreatedBy, WriteOffId: req.SourceId,
			AccountingDate: req.AccountingDate, Lines: req.Lines,
		})
		return err
	case inventoryapp.EventStocktakePosted:
		...
		_, err = d.Client.PostStocktakeJournal(ctx, &ledgerv1.PostStocktakeJournalRequest{
			RestaurantId: req.RestaurantId, CreatedBy: req.CreatedBy, StocktakeId: req.SourceId,
			AccountingDate: req.AccountingDate, Lines: req.Lines,
		})
		return err
	case inventoryapp.EventReceiptCancelled:
		_, err := d.Client.ReverseReceiptJournal(ctx, &ledgerv1.ReverseJournalRequest{RestaurantId: restaurantID, SourceId: sourceID})
		return err
	case inventoryapp.EventWriteOffCancelled:
		_, err := d.Client.ReverseWriteOffJournal(ctx, &ledgerv1.ReverseJournalRequest{RestaurantId: restaurantID, SourceId: sourceID})
		return err
	case inventoryapp.EventStocktakeCancelled:
		_, err := d.Client.ReverseStocktakeJournal(ctx, &ledgerv1.ReverseJournalRequest{RestaurantId: restaurantID, SourceId: sourceID})
		return err
	default:
